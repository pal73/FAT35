...
7073  0034 84            	pop	a
7074  0035 81            	ret
7118                     ; 139 void bin2bcd_int(unsigned short in) {
7119                     	switch	.text
...
7127                     ; 141 char i=5;
7129                     ; 143 for(i=0;i<5;i++)
7131  0038 0f01          	clr	(OFST+0,sp)
7132  003a               L5155:
7133                     ; 145 	dig[i]=in%10;
...
7163  0060 81            	ret
7200                     ; 150 void bcd2ind(void) {
7201                     	switch	.text
7202  0061               _bcd2ind:
7204  0061 88            	push	a
7205       00000001      OFST:	set	1
7208                     ; 153 for (i=4;i>0;i--)
7210  0062 a604          	ld	a,#4
7211  0064 6b01          	ld	(OFST+0,sp),a
7212  0066               L1455:
7213                     ; 155 	ind_out_[i-1]=DIGISYM[dig[i-1]];
...
7288                     ; 162 for (i=4;i>0;i--)
7290  008d a604          	ld	a,#4
7291  008f 6b01          	ld	(OFST+0,sp),a
7292  0091               L5655:
7293                     ; 164 	if(zero_on&&(!dig[i-1])&&(i-1))
...
7311                     ; 166 		ind_out_[i-1]=0b11111111;
...
7321  00b3               L3755:
7322                     ; 170 		ind_out_[i-1]=DIGISYM[dig[i-1]];
...
7351  00d5 84            	pop	a
7352  00d6 81            	ret
7388                     ; 176 void long2ind(unsigned long in) {
7389                     	switch	.text
7390  00d7               _long2ind:
7392       00000000      OFST:	set	0
7395                     ; 178 bin2bcd_long(in);
...
7597  0135               L1765:
7598                     ; 198 	for(i=0;i<len;i++) {
7600  0135 0f01          	clr	(OFST+0,sp)
7602  0137 2016          	jra	L7175
7603  0139               L3175:
7604                     ; 199 		ind_out[start+i]=ind_out_[i];
...
7631  0157 81            	ret
7689                     ; 205 void char2indh_sf(unsigned char in, char start, char fl) {
7690                     	switch	.text
7691  0158               _char2indh_sf:
7693  0158 89            	pushw	x
7694       00000000      OFST:	set	0
7697                     ; 208 	ind_out_[0]=DIGISYM[in%16];
...
7705                     ; 209 	ind_out_[1]=DIGISYM[in/16];
...
7818                     ; 229 		int2ind_slkuf(cnt_bFIRE_,0,3,0,1,0);
...
7829                     ; 230 		int2ind_slkuf(bFIRE_,3,1,0,1,0);
...
7896                     ; 248 			ind_out[1]=DIGISYM[21+(ind_ind_cnt%6)];
...
7906                     ; 249 			ind_out[2]=DIGISYM[21+(ind_ind_cnt%6)];
...
7916                     ; 250 			ind_out[3]=DIGISYM[21+(ind_ind_cnt%6)];
...
7928  0240               L7006:
7929                     ; 253 			int2ind_slkuf(EE_BONUS_SIGMA,0,4,0,1,2);
...
8034                     ; 279 		int2ind_slkuf(EE_FISK_CNT1,0,2,0,0,0);
...
8067                     ; 284 		if(sub_ind==0) {
8069  02f8 3d60          	tnz	_sub_ind
8070  02fa 2616          	jrne	L5306
8071                     ; 285 			int2ind_slkuf(888,0,3,0,1,0);
...
8103  032e               L1406:
8104                     ; 290 		else if(sub_ind==2) {
8106  032e b660          	ld	a,_sub_ind
8107  0330 a102          	cp	a,#2
8108  0332 2616          	jrne	L5406
8109                     ; 291 			int2ind_slkuf(EE_NUM_OF_BALLS,0,3,0,1,0);
...
8159  037e               L3306:
8160                     ; 302 	else if(ind==iWrk) {
8162  037e b662          	ld	a,_ind
8163  0380 a104          	cp	a,#4
8164  0382 262d          	jrne	L1606
8165                     ; 304 		int2ind_slkuf(EE_BONUS_SIGMA,0,4,0,1,0);
...
8322  044b               L3116:
8323                     ; 347 	else if(ind==iDeb) {
8325  044b b662          	ld	a,_ind
8326  044d a107          	cp	a,#7
8327  044f 2617          	jrne	L3216
8328                     ; 348 		int2ind_slkuf(target_plazma,0,3,0,1,0);
...
8343  0468               L3216:
8344                     ; 351 	else if(ind==iSerNum) {
8346  0468 b662          	ld	a,_ind
8347  046a a108          	cp	a,#8
8348  046c 2616          	jrne	L7216
8349                     ; 352 		int2ind_slkuf(SERIAL_NUMBER,0,4,0,0,0);
...
8572                     ; 406 				if(sub_ind>=4)sub_ind=0;
...
8629                     ; 419 				EE_PNEVMO_TIME = ((EE_PNEVMO_TIME/20)+1)*20;
...
8643  059f 85            	popw	x
8644                     ; 420 				if( (EE_PNEVMO_TIME>200) || (EE_PNEVMO_TIME<20) ) EE_PNEVMO_TIME=20;
8646  05a0 9c            	rvf
...
8667  05c4               L3126:
8668                     ; 423 			else if( (sub_ind==2) && (bFIRE) ) {
...
8704  05fb               L3226:
8705                     ; 427 			else if( (sub_ind==4) && (bFIRE) ) {
...
8739                     ; 438 				wrk_stat=wsON;
8741  0628 35010010      	mov	_wrk_stat,#1
8742                     ; 439 				wrk_cnt=120;
...
8755  063b               L7326:
8756                     ; 445 		else if((wrk_stat==wsON) && (!bERR)&&(!not_fire_cnt)) {
...
8957  06e9               L1336:
8958                     ; 516 while (tx_counter == TX_BUFFER_SIZE);
8960  06e9 c60000        	ld	a,_tx_counter
8961  06ec a150          	cp	a,#80
8962  06ee 27f9          	jreq	L1336
8963                     ; 518 if (tx_counter || ((UART1->SR & UART1_SR_TXE)==0))
...
8971  06fd               L7336:
8972                     ; 520    tx_buffer[tx_wr_index]=c;
8974  06fd 5f            	clrw	x
...
8982                     ; 521    if (++tx_wr_index == TX_BUFFER_SIZE) tx_wr_index=0;
...
9252  07ab 81            	ret
9284                     ; 580 void opto_drv (void) {
9285                     	switch	.text
9286  07ac               _opto_drv:
9290                     ; 581 	if((GPIOD->IDR)&(1<<1)) {
9292  07ac c65010        	ld	a,20496
9293  07af a502          	bcp	a,#2
9294  07b1 274c          	jreq	L3446
9295                     ; 582 		if(!bLINE_OLD)connect_cnt=1000;
...
9308                     ; 585 			if((zero_cnt>80)&&(zero_cnt<150))	{
...
9322                     ; 587 				if((wrk_stat==wsON)&&(!bERR)) {
...
9512                     ; 632 			if(target_cnt[i]<1000) target_cnt[i]++;
9514  087c 9c            	rvf
...
9533  089c               L7156:
9534                     ; 633 			if((target_cnt[i]==5)&& (!target_cnt_off[i])) {
...
9626                     ; 659 			if(target_cnt_off[i])target_cnt_off[i]--;
...
9742                     ; 679 		if(target_on[0]) EE_BONUS_SIGMA+=5;
...
9753  096d               L3656:
9754                     ; 680 		else if(target_on[1]) EE_BONUS_SIGMA+=10;
...
9765  097c               L7656:
9766                     ; 681 		else if(target_on[2]) EE_BONUS_SIGMA+=20;
...
9893                     ; 719 		if(wrk_cnt==0) {
9895  09bd be5a          	ldw	x,_wrk_cnt
9896  09bf 2620          	jrne	L1266
9897                     ; 720 			wrk_stat=wsOFF;
9899  09c1 3f10          	clr	_wrk_stat
9900                     ; 721 			if(EE_REMAIN_BALLS==EE_NUM_OF_BALLS)EE_FISK_CNT--;
...
9923                     ; 726 }
9926  09e1 81            	ret
9987                     ; 728 long delay_ms(short in) {
9988                     	switch	.text
9989  09e2               _delay_ms:
9991  09e2 520c          	subw	sp,#12
9992       0000000c      OFST:	set	12
9995                     ; 731 i=((long)in)*100UL;
...
10044  0a23 81            	ret
10067                     ; 740 void gpio_init(void){
10068                     	switch	.text
10069  0a24               _gpio_init:
10073                     ; 742 	GPIOB->CR1&=~((1<<0)|(1<<1)|(1<<2)|(1<<3));
10075  0a24 c65008        	ld	a,20488
10076  0a27 a4f0          	and	a,#240
10077  0a29 c75008        	ld	20488,a
10078                     ; 743 	GPIOB->CR2&=~((1<<0)|(1<<1)|(1<<2)|(1<<3));
10080  0a2c c65009        	ld	a,20489
10081  0a2f a4f0          	and	a,#240
10082  0a31 c75009        	ld	20489,a
10083                     ; 744 	GPIOB->DDR|=((1<<0)|(1<<1)|(1<<2)|(1<<3));
10085  0a34 c65007        	ld	a,20487
10086  0a37 aa0f          	or	a,#15
10087  0a39 c75007        	ld	20487,a
10088                     ; 747 	GPIOC->CR1&=~0xfe;
10090  0a3c c6500d        	ld	a,20493
10091  0a3f a401          	and	a,#1
10092  0a41 c7500d        	ld	20493,a
10093                     ; 748 	GPIOC->CR2&=~0xfe;
10095  0a44 c6500e        	ld	a,20494
10096  0a47 a401          	and	a,#1
10097  0a49 c7500e        	ld	20494,a
10098                     ; 749 	GPIOC->DDR|=0xfe;
10100  0a4c c6500c        	ld	a,20492
10101  0a4f aafe          	or	a,#254
10102  0a51 c7500c        	ld	20492,a
10103                     ; 750 	GPIOD->CR1&=~0x01;
10105  0a54 72115012      	bres	20498,#0
10106                     ; 751 	GPIOD->CR2&=~0x01;
10108  0a58 72115013      	bres	20499,#0
10109                     ; 752 	GPIOD->DDR|=0x01;	
10111  0a5c 72105011      	bset	20497,#0
10112                     ; 756 	GPIOB->DDR&=~(1<<5);	
10114  0a60 721b5007      	bres	20487,#5
10115                     ; 757 	GPIOB->CR1|=(1<<5);
10117  0a64 721a5008      	bset	20488,#5
10118                     ; 758 	GPIOB->CR2&=~(1<<5);
10120  0a68 721b5009      	bres	20489,#5
10121                     ; 771 	GPIOB->DDR&=~(1<<5);	
10123  0a6c 721b5007      	bres	20487,#5
10124                     ; 772 	GPIOB->CR1|=(1<<5);
10126  0a70 721a5008      	bset	20488,#5
10127                     ; 773 	GPIOB->CR2&=~(1<<5);
10129  0a74 721b5009      	bres	20489,#5
10130                     ; 775 	GPIOB->ODR|=0x0f;
10132  0a78 c65005        	ld	a,20485
10133  0a7b aa0f          	or	a,#15
10134  0a7d c75005        	ld	20485,a
10135                     ; 776 }
10138  0a80 81            	ret
10161                     ; 778 void t2_init(void){
10162                     	switch	.text
10163  0a81               _t2_init:
10167                     ; 779 	TIM2->PSCR = 0;
10169  0a81 725f530e      	clr	21262
10170                     ; 780 	TIM2->ARRH= 0x02;
10172  0a85 3502530f      	mov	21263,#2
10173                     ; 781 	TIM2->ARRL= 0x00;
10175  0a89 725f5310      	clr	21264
10176                     ; 782 	TIM2->CCR1H= 0x00;	
10178  0a8d 725f5311      	clr	21265
10179                     ; 783 	TIM2->CCR1L= 100;
10181  0a91 35645312      	mov	21266,#100
10182                     ; 784 	TIM2->CCR2H= 0x00;	
10184  0a95 725f5313      	clr	21267
10185                     ; 785 	TIM2->CCR2L= 200;
10187  0a99 35c85314      	mov	21268,#200
10188                     ; 786 	TIM2->CCR3H= 0x00;	
10190  0a9d 725f5315      	clr	21269
10191                     ; 787 	TIM2->CCR3L= 200;
10193  0aa1 35c85316      	mov	21270,#200
10194                     ; 790 	TIM2->CCMR2= ((7<<4) & TIM2_CCMR_OCM) | TIM2_CCMR_OCxPE; //OC2 toggle mode, prelouded
10196  0aa5 35785308      	mov	21256,#120
10197                     ; 796 	TIM2->CR1=(TIM2_CR1_CEN | TIM2_CR1_ARPE);	
10199  0aa9 35815300      	mov	21248,#129
10200                     ; 798 }
10203  0aad 81            	ret
10226                     ; 801 void t1_init(void){
10227                     	switch	.text
10228  0aae               _t1_init:
10232                     ; 802 	TIM1->ARRH= (u8)(AUTORELOAD >> 8);			// init pulse period - auto reload register
10234  0aae 35015262      	mov	21090,#1
10235                     ; 803 	TIM1->ARRL= (u8)(AUTORELOAD);
10237  0ab2 35a55263      	mov	21091,#165
10238                     ; 804 	TIM1->CCR1H= (u8)((AUTORELOAD/4) >> 8);	// init pulse wide (50% duty cycle) - compare register
10240  0ab6 725f5265      	clr	21093
10241                     ; 805 	TIM1->CCR1L= (u8)(AUTORELOAD/4);
10243  0aba 35695266      	mov	21094,#105
10244                     ; 806 	TIM1->CCR2H= (u8)((AUTORELOAD/8) >> 8);	// init pulse wide (50% duty cycle) - compare register
10246  0abe 725f5267      	clr	21095
10247                     ; 807 	TIM1->CCR2L= (u8)(AUTORELOAD/8);
10249  0ac2 35345268      	mov	21096,#52
10250                     ; 809 	TIM1->CCR3H= (u8)((AUTORELOAD/8) >> 8);	// init pulse wide (50% duty cycle) - compare register
10252  0ac6 725f5269      	clr	21097
10253                     ; 810 	TIM1->CCR3L= (u8)(AUTORELOAD/8);
10255  0aca 3534526a      	mov	21098,#52
10256                     ; 812 	TIM1->CCR4H= (u8)((AUTORELOAD/8) >> 8);	// init pulse wide (50% duty cycle) - compare register
10258  0ace 725f526b      	clr	21099
10259                     ; 813 	TIM1->CCR4L= (u8)(AUTORELOAD/8);
10261  0ad2 3534526c      	mov	21100,#52
10262                     ; 816 	TIM1->CCMR1= ((6<<4) & TIM1_CCMR_OCM) | TIM1_CCMR_OCxPE;	// OC1 PWM output compare mode 1
10264  0ad6 35685258      	mov	21080,#104
10265                     ; 817 	TIM1->CCMR2= ((6<<4) & TIM1_CCMR_OCM) | TIM1_CCMR_OCxPE;	// OC1 PWM output compare mode 1
10267  0ada 35685259      	mov	21081,#104
10268                     ; 818 	TIM1->CCMR3= ((6<<4) & TIM1_CCMR_OCM) | TIM1_CCMR_OCxPE;	// OC1 PWM output compare mode 1
10270  0ade 3568525a      	mov	21082,#104
10271                     ; 819 	TIM1->CCMR4= ((6<<4) & TIM1_CCMR_OCM) | TIM1_CCMR_OCxPE;	// OC1 PWM output compare mode 1
10273  0ae2 3568525b      	mov	21083,#104
10274                     ; 823 	TIM1->CCER1 = 0xff;//TIM1_CCER1_CC2E|TIM1_CCER1_CC1E|TIM1_CCER1_CC2P|TIM1_CCER1_CC1P;	// OC1 enable output pin
10276  0ae6 35ff525c      	mov	21084,#255
10277                     ; 825 	TIM1->CCER2 = 0xff;
10279  0aea 35ff525d      	mov	21085,#255
10280                     ; 826 	TIM1->BKR= 0x80;
10282  0aee 3580526d      	mov	21101,#128
10283                     ; 827 	TIM1->IER|= TIM1_IER_UIE;					// enable break interrupt
10285  0af2 72105254      	bset	21076,#0
10286                     ; 829 	TIM1->CR1= TIM4_CR1_URS|TIM1_CR1_CEN| TIM1_CR1_ARPE;					// timer 1 enable
10288  0af6 35855250      	mov	21072,#133
10289                     ; 832 }
10292  0afa 81            	ret
10315                     ; 834 void spi_init(void){
10316                     	switch	.text
10317  0afb               _spi_init:
10321                     ; 835 	SPI->CR1= /*SPI_CR1_LSBFIRST |*/
10321                     ; 836 			SPI_CR1_SPE | 
10321                     ; 837 			( (3<< 2) & SPI_CR1_BR ) |
10321                     ; 838 			SPI_CR1_MSTR |
10321                     ; 839 			SPI_CR1_CPOL |
10321                     ; 840 			SPI_CR1_CPHA; 
10323  0afb 354f5200      	mov	20992,#79
10324                     ; 842 	SPI->CR2= SPI_CR2_SSM | SPI_CR2_SSI;
10326  0aff 35035201      	mov	20993,#3
10327                     ; 843 	SPI->ICR= 0;	
10329  0b03 725f5202      	clr	20994
10330                     ; 844 }
10333  0b07 81            	ret
10376                     ; 846 char spi(char in){
10377                     	switch	.text
10378  0b08               _spi:
...
10385  0b0a               L5576:
10386                     ; 848 	while(!((SPI->SR)&SPI_SR_TXE));
...
10396  0b16               L5676:
10397                     ; 850 	while(!((SPI->SR)&SPI_SR_RXNE));
...
10559                     ; 876 		GPIOD->ODR&=ind_out[3-ind_cnt]|0xfe;
...
10927                     ; 1008 		if (++tx_rd_index == TX_BUFFER_SIZE) tx_rd_index=0;
...
11002                     ; 1028 		if (++rx_wr_index == RX_BUFFER_SIZE) rx_wr_index=0;
...
11013  0d23               L1317:
11014                     ; 1029 		if (++rx_counter == RX_BUFFER_SIZE){
...
11021                     ; 1030 			rx_counter=0;
11023  0d2f 5f            	clrw	x
11024  0d30 bf09          	ldw	_rx_counter,x
11025                     ; 1031 			rx_buffer_overflow=1;
...
11100                     	switch	.text
11101  0d37               _main:
11103  0d37 89            	pushw	x
11104       00000002      OFST:	set	2
11107                     ; 1041 	CLK->CKDIVR=0;
11109  0d38 725f50c6      	clr	20678
11110                     ; 1043 	gpio_init();
11112  0d3c cd0a24        	call	_gpio_init
11114                     ; 1047 	FLASH_DUKR=0xae;
11116  0d3f 35ae5064      	mov	_FLASH_DUKR,#174
11117                     ; 1048 	FLASH_DUKR=0x56;	
11119  0d43 35565064      	mov	_FLASH_DUKR,#86
11120                     ; 1092 	if(EE_REMAIN_BALLS) {
11122  0d47 ce4040        	ldw	x,_EE_REMAIN_BALLS
11123  0d4a 270f          	jreq	L3517
11124                     ; 1096 		ind=iWait1;
11126  0d4c 350c0062      	mov	_ind,#12
11127                     ; 1097 		ret_ind=iWait2;
11129  0d50 350d0061      	mov	_ret_ind,#13
11130                     ; 1098 		ret_cnt1=1200;
11132  0d54 ae04b0        	ldw	x,#1200
11133  0d57 bf15          	ldw	_ret_cnt1,x
11135  0d59 2003          	jra	L5517
11136  0d5b               L3517:
11137                     ; 1100 	else in_button();
11139  0d5b cd0755        	call	_in_button
11141  0d5e               L5517:
11142                     ; 1105 	t4_init();
11144  0d5e cd0b26        	call	_t4_init
11146                     ; 1106 	enableInterrupts();	
11149  0d61 9a            rim
11151                     ; 1109 uart_init();
11154  0d62 cd0724        	call	_uart_init
11156  0d65               L7517:
11157                     ; 1115 	if(bRXIN)	
11159  0d65 3d01          	tnz	_bRXIN
11160  0d67 2705          	jreq	L3617
11161                     ; 1117 		bRXIN=0;
11163  0d69 3f01          	clr	_bRXIN
11164                     ; 1119 		uart_in();
11166  0d6b cd0754        	call	_uart_in
11168  0d6e               L3617:
11169                     ; 1122 	if(b1000Hz)
11171  0d6e 725d0000      	tnz	_b1000Hz
11172  0d72 271b          	jreq	L5617
11173                     ; 1124 		b1000Hz=0;
11175  0d74 725f0000      	clr	_b1000Hz
11176                     ; 1125 		opto_drv();
11178  0d78 cd07ac        	call	_opto_drv
11180                     ; 1126 		target_drv();
11182  0d7b cd083a        	call	_target_drv
11184                     ; 1127 		target_an();
11186  0d7e cd0950        	call	_target_an
11188                     ; 1128 		rele_drv();
11190  0d81 cd0990        	call	_rele_drv
11192                     ; 1129 		if(not_fire_cnt)not_fire_cnt--;
11194  0d84 be0f          	ldw	x,_not_fire_cnt
11195  0d86 2707          	jreq	L5617
11198  0d88 be0f          	ldw	x,_not_fire_cnt
11199  0d8a 1d0001        	subw	x,#1
11200  0d8d bf0f          	ldw	_not_fire_cnt,x
11201  0d8f               L5617:
11202                     ; 1132 	if(b100Hz)
11204  0d8f 725d0001      	tnz	_b100Hz
11205  0d93 270d          	jreq	L1717
11206                     ; 1134 		b100Hz=0;
11208  0d95 725f0001      	clr	_b100Hz
11209                     ; 1136 		ibatton_drv();
11211  0d99 cd0000        	call	_ibatton_drv
11213                     ; 1137 		ibatton_an();
11215  0d9c cd0000        	call	_ibatton_an
11217                     ; 1138 		but_an();
11219  0d9f cd051f        	call	_but_an
11221  0da2               L1717:
11222                     ; 1141 	if(b10Hz)
11224  0da2 725d0002      	tnz	_b10Hz
11225  0da6 2753          	jreq	L3717
11226                     ; 1143 		b10Hz=0;
11228  0da8 725f0002      	clr	_b10Hz
11229                     ; 1146 		ind_hndl();
11231  0dac cd01a7        	call	_ind_hndl
11233                     ; 1147 		if(ret_cnt)
11235  0daf be5d          	ldw	x,_ret_cnt
11236  0db1 270e          	jreq	L5717
11237                     ; 1149 			ret_cnt--;
11239  0db3 be5d          	ldw	x,_ret_cnt
11240  0db5 1d0001        	subw	x,#1
11241  0db8 bf5d          	ldw	_ret_cnt,x
11242                     ; 1150 			if(ret_cnt==0)
11244  0dba be5d          	ldw	x,_ret_cnt
11245  0dbc 2603          	jrne	L5717
11246                     ; 1152 				ind=ret_ind;
11248  0dbe 456162        	mov	_ind,_ret_ind
11249  0dc1               L5717:
11250                     ; 1155 		if(ret_cnt1)
11252  0dc1 be15          	ldw	x,_ret_cnt1
11253  0dc3 2736          	jreq	L3717
11254                     ; 1157 			ret_cnt1--;
11256  0dc5 be15          	ldw	x,_ret_cnt1
11257  0dc7 1d0001        	subw	x,#1
11258  0dca bf15          	ldw	_ret_cnt1,x
11259                     ; 1158 			if(ret_cnt1==0)
11261  0dcc be15          	ldw	x,_ret_cnt1
11262  0dce 262b          	jrne	L3717
11263                     ; 1160 				ind=iWait2;
11265  0dd0 350d0062      	mov	_ind,#13
11266                     ; 1161 				EE_FISK_CNT1++;
11268  0dd4 ce4108        	ldw	x,_EE_FISK_CNT1
11269  0dd7 1c0001        	addw	x,#1
11270  0dda cf4108        	ldw	_EE_FISK_CNT1,x
11271                     ; 1162 				if(EE_FISK_CNT1>=100)EE_FISK_CNT1=0;
11273  0ddd 9c            	rvf
11274  0dde ce4108        	ldw	x,_EE_FISK_CNT1
11275  0de1 a30064        	cpw	x,#100
11276  0de4 2f09          	jrslt	L5027
11279  0de6 5f            	clrw	x
11280  0de7 89            	pushw	x
11281  0de8 ae4108        	ldw	x,#_EE_FISK_CNT1
11282  0deb cd0000        	call	c_eewrw
11284  0dee 85            	popw	x
11285  0def               L5027:
11286                     ; 1163 				EE_REMAIN_BALLS=0;
11288  0def 5f            	clrw	x
11289  0df0 89            	pushw	x
11290  0df1 ae4040        	ldw	x,#_EE_REMAIN_BALLS
11291  0df4 cd0000        	call	c_eewrw
11293  0df7 85            	popw	x
11294                     ; 1164 				ret_cnt=0;
11296  0df8 5f            	clrw	x
11297  0df9 bf5d          	ldw	_ret_cnt,x
11298  0dfb               L3717:
11299                     ; 1170 	if(b5Hz)
11301  0dfb 725d0003      	tnz	_b5Hz
11302  0dff 2704          	jreq	L7027
11303                     ; 1172 		b5Hz=0;
11305  0e01 725f0003      	clr	_b5Hz
11306  0e05               L7027:
11307                     ; 1176 	if(b2Hz)
11309  0e05 725d0004      	tnz	_b2Hz
11310  0e09 2704          	jreq	L1127
11311                     ; 1178 		b2Hz=0;
11313  0e0b 725f0004      	clr	_b2Hz
11314  0e0f               L1127:
11315                     ; 1182 	if(b1Hz)
11317  0e0f 725d0005      	tnz	_b1Hz
11318  0e13 2603          	jrne	L671
11319  0e15 cc0d65        	jp	L7517
11320  0e18               L671:
11321                     ; 1185 		b1Hz=0;
11323  0e18 725f0005      	clr	_b1Hz
11324                     ; 1190 		uart_buff="Hello %d,%x,%d,%d,%x\r\n";
11326                     ; 1192 		puts("Hello"+plazma[0]);
11328  0e1c be13          	ldw	x,_plazma
11329  0e1e 1c0022        	addw	x,#L7127
11330  0e21 cd0000        	call	_puts
11332                     ; 1194 		wrk_hndl();
11334  0e24 cd09b2        	call	_wrk_hndl
11336  0e27 ac650d65      	jpf	L7517
12468                     	xdef	_main
12469                     	xdef	f_UARTRxInterrupt
12470                     	xdef	f_UARTTxInterrupt
12471                     	xdef	f_TIM1_Ovf_Interrupt
12472                     	xdef	_t1_init
12473                     	xdef	_wrk_hndl
12474                     	xdef	_rele_drv
12475                     	xdef	_target_an
12476                     	xdef	_target_drv
12477                     	xdef	_opto_drv
12478                     	xdef	_in_button
12479                     	xdef	_uart_in
12480                     	xdef	_uart_init
12481                     	xdef	_putchar
12482                     	xdef	_but_an
12483                     	xdef	_ind_hndl
12484                     	xdef	_int2ind_slkuf
12485                     	xdef	_bcd2ind
12486                     	xdef	_long2ind
12487                     	xdef	_bcd2ind_zero
12488                     	xdef	_bin2bcd_int
12489                     	xdef	_bin2bcd_long
12490                     	xdef	_plazma
12491                     	switch	.ubsct
12492  0000               _rx_buffer_overflow:
12493  0000 00            	ds.b	1
12494                     	xdef	_rx_buffer_overflow
12495  0001               _bRXIN:
12496  0001 00            	ds.b	1
12497                     	xdef	_bRXIN
12498  0002               _rx_data:
12499  0002 00            	ds.b	1
12500                     	xdef	_rx_data
12501  0003               _rx_status:
12502  0003 00            	ds.b	1
12503                     	xdef	_rx_status
12504  0004               _bOUT_FREE:
12505  0004 00            	ds.b	1
12506                     	xdef	_bOUT_FREE
12507  0005               _rx_rd_index:
12508  0005 0000          	ds.b	2
12509                     	xdef	_rx_rd_index
12510  0007               _rx_wr_index:
12511  0007 0000          	ds.b	2
12512                     	xdef	_rx_wr_index
12513  0009               _rx_counter:
12514  0009 0000          	ds.b	2
12515                     	xdef	_rx_counter
12516                     	xdef	_rx_buffer
12517  000b               _tx_rd_index:
12518  000b 00            	ds.b	1
12519                     	xdef	_tx_rd_index
12520  000c               _tx_wr_index:
12521  000c 00            	ds.b	1
12522                     	xdef	_tx_wr_index
12523                     	switch	.bss
12524  0000               _tx_counter:
12525  0000 00            	ds.b	1
12526                     	xdef	_tx_counter
12527                     	xdef	_tx_buffer
12528                     	switch	.ubsct
12529  000d               _plazma_SS:
12530  000d 0000          	ds.b	2
12531                     	xdef	_plazma_SS
12532  000f               _not_fire_cnt:
12533  000f 0000          	ds.b	2
12534                     	xdef	_not_fire_cnt
12535  0011               _opto_cnt:
12536  0011 0000          	ds.b	2
12537                     	xdef	_opto_cnt
12538  0013               _target_plazma:
12539  0013 00            	ds.b	1
12540                     	xdef	_target_plazma
12541  0014               _ind_ind_cnt:
12542  0014 00            	ds.b	1
12543                     	xdef	_ind_ind_cnt
12544  0015               _ret_cnt1:
12545  0015 0000          	ds.b	2
12546                     	xdef	_ret_cnt1
12547  0017               _ind_out_:
12548  0017 0000000000    	ds.b	5
12549                     	xdef	_ind_out_
12550  001c               _bFL_:
12551  001c 00            	ds.b	1
12552                     	xdef	_bFL_
12553  001d               _bFL1:
12554  001d 00            	ds.b	1
12555                     	xdef	_bFL1
12556  001e               _bFL2:
12557  001e 00            	ds.b	1
12558                     	xdef	_bFL2
12559  001f               _bFL5:
12560  001f 00            	ds.b	1
12561                     	xdef	_bFL5
12562                     	xdef	_bERR
12563                     	xdef	_ind_on_cnt1
12564                     	xdef	_ind_on_cnt
12565  0020               _zero_on:
12566  0020 00            	ds.b	1
12567                     	xdef	_zero_on
12568  0021               _dig:
12569  0021 000000000000  	ds.b	10
12570                     	xdef	_dig
12571                     	xdef	_DIGISYM
12572                     	xdef	_ind_out
12573  002b               _ind_cnt:
12574  002b 00            	ds.b	1
12575                     	xdef	_ind_cnt
12576  002c               _parol_sigma:
12577  002c 0000          	ds.b	2
12578                     	xdef	_parol_sigma
12579  002e               _cnt_bFIRE_:
12580  002e 0000          	ds.b	2
12581                     	xdef	_cnt_bFIRE_
12582  0030               _cnt_bFIRE1:
12583  0030 00            	ds.b	1
12584                     	xdef	_cnt_bFIRE1
12585  0031               _cnt_bFIRE:
12586  0031 00            	ds.b	1
12587                     	xdef	_cnt_bFIRE
12588  0032               _cnt_bKEY:
12589  0032 00            	ds.b	1
12590                     	xdef	_cnt_bKEY
12591  0033               _bFIRE_BLOCK:
12592  0033 00            	ds.b	1
12593                     	xdef	_bFIRE_BLOCK
12594  0034               _bFIRE1:
12595  0034 00            	ds.b	1
12596                     	xdef	_bFIRE1
12597  0035               _bFIRE_:
12598  0035 00            	ds.b	1
12599                     	xdef	_bFIRE_
12600  0036               _bFIRE:
12601  0036 00            	ds.b	1
12602                     	xdef	_bFIRE
12603  0037               _bKEY:
12604  0037 00            	ds.b	1
12605                     	xdef	_bKEY
12606  0038               _zatvor_cnt:
12607  0038 0000          	ds.b	2
12608                     	xdef	_zatvor_cnt
12609                     	xdef	_bZATVOR
12610                     	xdef	_rel_out_cnt
12611                     	xdef	_but_cnt
12612                     .eeprom:	section	.data
12613  0000               _ee_plazma:
12614  0000 00000000      	ds.b	4
12615                     	xdef	_ee_plazma
12616  0004               _ee_dumm:
12617  0004 000000000000  	ds.b	10
12618                     	xdef	_ee_dumm
12619                     	xdef	_t0_cnt4
12620                     	xdef	_b2Hz
12621                     	xdef	_b1000Hz
12622                     	xref	_puts
12623                     	xdef	f_TIM4_UPD_Interrupt
12624                     	xdef	_t4_init
12625                     	xdef	_spi_init
12626                     	xdef	_t2_init
12627                     	xdef	_gpio_init
12628                     	xdef	_delay_ms
12629                     	xdef	_spi
12630                     	xdef	_char2indh_sf
12631                     	xdef	_plazma_ibatton2
12632                     	xdef	_plazma_ibatton1
12633                     	switch	.ubsct
12634  003a               _connect_cnt:
12635  003a 0000          	ds.b	2
12636                     	xdef	_connect_cnt
12637  003c               _zero_cnt:
12638  003c 0000          	ds.b	2
12639                     	xdef	_zero_cnt
12640  003e               _one_cnt:
12641  003e 0000          	ds.b	2
12642                     	xdef	_one_cnt
12643  0040               _bLINE_OLD:
12644  0040 00            	ds.b	1
12645                     	xdef	_bLINE_OLD
12646  0041               _bIN_OLD:
12647  0041 00            	ds.b	1
12648                     	xdef	_bIN_OLD
12649  0042               _rele_cnt:
12650  0042 0000          	ds.b	2
12651                     	xdef	_rele_cnt
12652  0044               _fire_cnt:
12653  0044 0000          	ds.b	2
12654                     	xdef	_fire_cnt
12655  0046               _target_on:
12656  0046 00000000      	ds.b	4
12657                     	xdef	_target_on
12658  004a               _target_cnt_off:
12659  004a 000000000000  	ds.b	8
12660                     	xdef	_target_cnt_off
12661  0052               _target_cnt:
12662  0052 000000000000  	ds.b	8
12663                     	xdef	_target_cnt
12664  005a               _wrk_cnt:
12665  005a 0000          	ds.b	2
12666                     	xdef	_wrk_cnt
12667                     	xdef	_wrk_stat
12668                     	switch	.bss
12669  0001               _rx_plazma:
12670  0001 00            	ds.b	1
12671                     	xdef	_rx_plazma
12672                     	switch	.ubsct
12673  005c               _i_ser_num_cnt:
12674  005c 00            	ds.b	1
12675                     	xdef	_i_ser_num_cnt
12676  005d               _ret_cnt:
12677  005d 0000          	ds.b	2
12678                     	xdef	_ret_cnt
12679  005f               _fin_ind_cnt:
12680  005f 00            	ds.b	1
12681                     	xdef	_fin_ind_cnt
12682  0060               _sub_ind:
12683  0060 00            	ds.b	1
12684                     	xdef	_sub_ind
12685  0061               _ret_ind:
12686  0061 00            	ds.b	1
12687                     	xdef	_ret_ind
12688  0062               _ind:
12689  0062 00            	ds.b	1
12690                     	xdef	_ind
12691                     	switch	.bss
12692  0002               _bSTART:
12693  0002 00            	ds.b	1
12694                     	xdef	_bSTART
12695                     	xdef	_t0_cnt3
12696                     	xdef	_t0_cnt2
12697                     	xdef	_t0_cnt1
...
12701                     	xdef	_b10Hz
12702                     	xdef	_b100Hz
12703                     	xref	_ibatton_an
12704                     	xref	_ibatton_drv
12705                     	switch	.const
12706  0022               L7127:
12707  0022 48656c6c6f00  	dc.b	"Hello",0
12708  0028               L5127:
12709  0028 48656c6c6f20  	dc.b	"Hello %d,%x,%d,%d,"
12710  003a 25780d        	dc.b	"%x",13
12711  003d 0a00          	dc.b	10,0
12712                     	xref.b	c_lreg
12713                     	xref.b	c_x
12714                     	xref.b	c_y
12734                     	xref	c_lcmp
